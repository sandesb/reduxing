...
export const EDITOR_JS_TOOLS = {
 ...
 attaches: {
    ...
    config: {
      uploader: {
        async uploadByFile(file) {
          // Upload file (PDF, MP4, etc.) to Cloudinary
          const url = await uploadImageToCloudinary(file);

          if (!url) throw new Error('File upload failed');

          // Ensure the returned format for AttachesTool
          return {
            success: 1,
            file: {
              ...
              extension: file.name.split('.').pop(),  // File extension (e.g., pdf, mp4)
            },
          };
        },
      },
      accept: 'application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,video/mp4',  // Allowed file types including mp4
    },
  },
    // other tools...
  };